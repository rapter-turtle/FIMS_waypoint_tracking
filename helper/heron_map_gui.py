...
class SensorFusionEKF(Node):
    def __init__(self):
        ...
        self.ekf_sub = self.create_subscription(Float64MultiArray, '/bada/estimated_state', self.ekf_callback, 10)
        self.thrust_sub = self.create_subscription(Float64MultiArray, '/bada/ros_actuator', self.thrust_callback, 10)
        self.waypoint_sub = self.create_subscription(Waypoint, "/bada/waypoints", self.waypoints_callback, 10)
    ...
    def ekf_callback(self, msg):  # - 주기가 gps callback 주기랑 같음 - gps data callback받으면 ekf에서 publish 하기때문
        ...
        self.x = msg.data[0]
        self.y = msg.data[1]
        # self.p = msg.data[2]-msg->data[2]+0.5*3.141592;
        ...
        self.p_sensor = -(msg.data[8]-0.5*3.141592)
